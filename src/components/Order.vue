      ...
      <div class="order">
        <div class="form">
          <div class="group">
            ...
            <div class="row">
              <label>手&emsp;&emsp;機</label>
              <el-input v-model="form.phone" placeholder></el-input>
            </div>
            <div class="row">
              <label>驗&ensp;證&ensp;碼</label>
        ...
        <div class="control privacy">
          <p>你所登錄的個人資料將做為以下用途</p>
          <p>
            (一)本網站所載之相關事項通知 
            <br>(二)客戶管理與服務
            <br>(三)本公司行銷業務之推廣本案實際內容以現場公布為準
          </p>
          <el-checkbox v-model="form.checked">
            <h3>
              本人已知悉以上個人資料蒐集聲明事項<br>
            </h3>
          </el-checkbox>
  ...
  methods: {
    ...
    submit() {
      if (this.form.name == "") {
        ...
        const utmCampaign = urlParams.get("utm_campaign") || "lilingtienmu";

        let reqData = "Name=" + this.form.name + "&Email=" + this.form.email + "&MobilePhone=" + this.form.phone + "&City=" + this.form.city + "&Dist=" + this.form.area + "&SmsVerifyCode=" + this.form.captcha + "&ProjectId=a0B6F00001m71Pq&utmSource=" + utmSource + "&utmMedium=" + utmMedium + "&utmCampaign=" + utmCampaign + "&utmContent=" + utmContent;
        ...
        fetch("https://www.hiyes.tw/BuildingCase/Booking", {
          ...
          if(!jsonData.IsError){
            ...
            //   }`;
            window.location.href = "thanku.html?utm_campaign=" + utmCampaign;
          }else{
            let resultMsg = jsonData.Message.ResultMessage;
            this.showNotify("提醒", resultMsg, "error");
          }
        }).catch((err) => {
          console.log('錯誤:', err);
        });
    ...
    getMobileValidCode() {
      ...
      fetch("https://www.hiyes.tw/Auth/SendSmsVerifyCode", {
        method: "POST",
        body: "mobilePhone=" + this.form.phone,
        // body: JSON.stringify(data),
        headers: {
          "content-type": "application/x-www-form-urlencoded",
        },
        ...
        if(!jsonData.IsError){
          this.showNotify("提醒", "已發出驗證碼至您手機", "success");
        }
      }).catch((err) => {
        console.log('錯誤:', err);
      });
