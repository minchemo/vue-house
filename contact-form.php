...
$reservation_people_num       = isset($_POST['reservation_people_num']) ? $_POST['reservation_people_num'] : '';
...
$time_end        = isset($_POST['time_end']) ? $_POST['time_end'] : '';
    ...
    */
    ...
    $mail->Body = "網站：https://" . $src . "/<BR>姓名：" . $name . "<BR>電話：" . $phone . "<BR>城市：" . $city . $area . "<BR>需求房型：".$room_type."<BR>購屋預算：".$budget."<BR>預約日期：".$reservation_date."<BR>預約時段：".$reservation_time."<BR>預約人數：".$reservation_people_num."<BR>留言：".$msg."<BR>備註："."<BR><BR>填表日期：".$datetime."<BR>廣告來源：".$utm_source."<BR>廣告媒介：".$utm_medium."<BR>廣告名稱：".$utm_campaign."<BR>廣告內容：".$utm_content; //設定郵件內容
    ...
    if ($bCheck == true) { //if start
      ...
      # 添加到 Googlde 資料DB Start
        try {
            ...
            $url .= "&reservation_people_num=".$reservation_people_num;
