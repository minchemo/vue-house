...
$case_name = $dataList[0]['casename'];
    ...
    $info_source   = isset($_POST['info_source']) ? $_POST['info_source'] : '';
    ...
    $mail->Body = "網站：https://" . $src . "/<BR>姓名：".$name."<BR>電話：".$phone."<BR>城市：".$city.$area."<BR>性別：".$gender."<BR>資料來源：".$info_source."<BR>房型需求：".$houseStyle."<BR>車位需求：".$parking."<BR>留言：".$msg."<BR><BR>填表日期：".$datetime."<BR>廣告來源：".$utm_source."<BR>廣告媒介：".$utm_medium."<BR>廣告名稱：".$utm_campaign."<BR>廣告內容：".$utm_content; //設定郵件內容
    ...
    if ($bCheck == true) { //if start
        ...
        try {
            ...
            $url .= "&city=".$city;
            $url .= "&area=".$area;
            $url .= "&message=性別：".$gender."；資料來源：".$info_source."；房型需求：".$houseStyle."；車位需求：".$parking."；留言：".$msg;
