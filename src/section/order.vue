<template>
  <div id="order" class="order relative bg-[#fff] text-center">
    ...
    <img v-else class="order-title-img" src="@/section/form/titleImg.svg" alt="泰嘉拓真" srcset="" data-aos="fade"
      ...
      <!-- Form -->
      <div class="form mx-auto relative flex justify-center" data-aos="zoom-in">
...
<template v-for="(fieldData, fieldKey) in selectFields" :key="fieldKey">
    <label class="row w-full">
      ...
      <select
        class="select w-full rounded-none input-style select-style"
...
<!-- 動態 select end-->
        ...
        <div class="right">
          <textarea :value="formData.msg" @input="(event) => (formData.msg = event.target.value)"
            class="row textarea w-full h-full rounded-none input-style select-style" placeholder="備註訊息"></textarea>
        </div>
      </div>
  ...
  submitted.value = true;
    ...
    */
    fetch(
      ...
      &msg=${formData.budget}；${formData.msg}
