...
const send = async () => {
  ...
  presendA.form.utm_source = utmSource;
  presendA.form.utm_medium = utmMedium;
  presendA.form.utm_content = utmContent;
  presendA.form.utm_campaign = utmCampaign;
  ...
  for (const [key, value] of Object.entries(formData)) {
    if (key !== "r_verify" && key !== "policyChecked") {
      presendB.append(key, value);
    }
  }

  presendB.append("utm_source", utmSource);
  presendB.append("utm_medium", utmMedium);
  presendB.append("utm_content", utmContent);
  presendB.append("utm_campaign", utmCampaign);
  presendB.append("message", formData.msg);
  presendB.append("case_code", info.case_code ? info.case_code : info.caseidB? info.caseidB: info.caseidA);

  try {
    // ===== Google Sheet（保留）=====
    fetch(`https://script.google.com/macros/s/AKfycbzqyW-sbiYwNAwunTDkp3ncVcvPnPEkvsUQWswyprd2b1V2u1HQ/exec?name=${formData.name}
